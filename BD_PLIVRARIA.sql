...
/*CRIANDO A PROCEDURE QUE CONCATENA O PREÇO DO LIVRO E COM A EDITORA USANDO INNER JOIN */

DELIMITER $$

CREATE PROCEDURE CONSULTALIVROS(vEditora VARCHAR(50))
BEGIN

SELECT CONCAT(' O LIVRO  ' , NOME_LIVRO ,   ' CUSTA ' , PRECO) AS PRECO_EDITORA
FROM LIVRO

INNER JOIN EDITORA
ON LIVRO.ID_EDITORA = EDITORA.ID_EDITORA
WHERE EDITORA.NOME_EDITORA = vEditora;

END $$

CALL CONSULTALIVROS('SARAIVA');
